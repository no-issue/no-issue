...
import zipfile
import ntpath
import GATracker, uuid

homePath = xbmc.translatePath('special://home')
userData = xbmc.translatePath('special://profile/')
addonPath = os.path.join(os.path.join(homePath, 'addons'),'plugin.video.ultimateinstaller')
mediaPath = os.path.join(addonPath, 'media')
addonName = "Ultimate Installer"

def mainMenu():
	global analytics
	analytics.sendPageView("UltimateInstaller","mainmenu","main")

	xbmc.executebuiltin("Container.SetViewMode(500)")
	addItem('HTPC','url', 1, os.path.join(mediaPath, "htpc.png"))
	addItem('Android','url', 2, os.path.join(mediaPath, "droid.png"))
	addItem('Install NI news ticker','url', 3, os.path.join(mediaPath, "rssfeed.png"))
...
def setupAnalytics():
	...
	analytics = GATracker.GAconnection("UA-36482713-9", userID)

ADDON=xbmcaddon.Addon(id='plugin.video.ultimateinstaller')
dialog = xbmcgui.Dialog()    
VERSION = "0.0.1"
PATH = "Ultimate Installer"            

    
def CATEGORIES():
    global analytics
    analytics.sendEvent("UltimateInstaller", "downloading")
    link = OPEN_URL('http://pastebin.com/raw.php?i=nzPBcY96').replace('\n','').replace('\r','')
...
def wizard(name,url,description):
    global analytics
    analytics.sendEvent("UltimateInstaller", "wizard")
    path = xbmc.translatePath(os.path.join('special://home/addons','packages'))
    dp = xbmcgui.DialogProgress()
    dp.create("Ultimate Installer","Downloading ",'', 'Please Wait')
